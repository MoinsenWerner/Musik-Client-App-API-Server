...
The separate `chat.db`, managed directly with `sqlite3` by `chat.py`, contains `chat_uploads`, `chat_profiles`, `chat_user_settings`, `chat_presence`, `chat_blocks`, `chat_clears`, `chat_groups`, `chat_group_members`, `chat_messages`, `chat_message_attachments`, `chat_message_receipts`, and `chatgpt_messages`. Profiles/settings hold the image, display name, and presence-privacy choice; presence timestamps distinguish online, idle, and offline; blocks stop direct messages; clear markers hide older history only for one user; and receipts track delivery/read state. Attachment bytes live in `chat_uploads/`; the ordered many-to-many `chat_message_attachments` table allows one message to contain any number of images, videos, and files. Startup copies legacy single `file_upload_id`/`image_upload_id` associations into that table. Foreign keys connect messages to groups, uploads, and optional replied-to message rows. Direct messages store a recipient, while group messages store a group ID. ChatGPT prompts and answers are stored per username so later model requests can include that user's prior messages. Startup adds `chat_messages.reply_to_message_id` to older chat databases when needed.
...
- **GET `/webchat`** — Renders the WhatsApp-inspired browser chat UI. A blocking login overlay exchanges gateway client credentials at `/token`; after successful login the UI exposes direct/self/group messages, group membership, media, histories, and ChatGPT. An in-app launch may supply the exact query combination `caller=in-app`, `client-id=<ID>`, and `client-secret=<secret>` to perform the same token exchange automatically without showing the overlay. This launch also sets a persistent in-page mobile mode before the credential query is removed from the URL: on viewports up to 900 px the chat list occupies the full screen, opening a conversation/subpage switches to a full-screen detail view, and back buttons return to the list. Because the mode is stored as a body class rather than inferred from the current URL, it survives all in-page chat/subpage navigation after `history.replaceState` removes the launch query. A failed exchange restores the login overlay. Attachments are selected directly in the composer, uploaded immediately before message creation, and never expose their response IDs in the UI. Message type is inferred from text, URL, and attachment MIME type. Images/videos render inline, open in a full-screen viewer, and support context-menu reply/download actions. In standalone `chat.py` mode, `/token` deliberately returns 503 because gateway clients live in the main database, so automatic or manual authenticated use requires `servus.py`.
...
- **GET `/chat/share/playlists/<username>`** — Returns playlist metadata and songs from the main playlist database, divided into all server playlists and playlists whose creator matches the username. Standalone mode returns empty lists. The webchat uses it for song/playlist sharing.
...
- **POST `/chat/group/<int:group_id>/members`** — Changes membership using required query `action=add|remove`, `username`, and `ersteller`. Only the stored group creator may change membership; the creator cannot remove themselves.
- **GET `/chat/group/<int:group_id>?username=<username>`** — Returns group name, creator, and the complete member list when the requesting username belongs to the group. The webchat opens this information when a group name is clicked.
...
The webchat constrains itself to the dynamic viewport height: its composer remains fixed at the bottom while only the message history scrolls. Existing direct chats, self-chats, and groups appear in a WhatsApp-like list with last-message previews, per-chat unread counts, receipt checkmarks, and privacy-aware presence dots, and open when clicked. The header's three-dot menu provides Info, refresh, per-user clear, and block/unblock. Info shows media previews, links, common groups, clear, and block controls. The top `+` toggles the otherwise hidden new-chat form; the gear opens display-name, presence-privacy, and profile settings. Users can add, replace, and remove their profile picture. Clicking an open group's name shows all members; its creator additionally receives add/remove controls. OAuth client IDs and chat-known identities are loaded into the recipient dropdown and multi-select group-member fields after login. The composer `+` menu separates images/videos, files, songs, and playlists. The attachment picker accepts multiple selections, uploads every selected item immediately before sending, and renders all images/videos/files in the resulting message. Song/playlist sharing distinguishes server playlists from playlists created by the signed-in user and can share one or multiple selected songs as Spotify identifiers in message text. The Groups tab contains its own creation form and lists the signed-in user's groups. A four-second update/presence poll displays unread state and page-title count; when browser notification permission is granted, background tabs also receive a system notification.
