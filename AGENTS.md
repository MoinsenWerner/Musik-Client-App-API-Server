...
- Spotify redirect URI: `https://api.plsreload.de/callback`.
...
The same routes below `/auth` are `/auth/health`, `/auth/register`, `/auth/get`, `/auth/.well-known/webauthn`, and `/auth/api/...`. WebAuthn requires stable `VAULT_KEY` and `SECRET_KEY` values. `RP_ID` and public HTTPS `ORIGIN` may be configured with `source auth/set-secrets.sh`. For every options request, the app uses the browser `Origin` when available and only accepts a configured RP ID when it equals the browser host or is a domain suffix of it; otherwise it falls back to the browser host. The selected RP ID and origin are stored with the challenge and reused for verification. This prevents stale environment variables from producing the browser's “relying party ID is not a registrable domain suffix” error. The configured/derived origin is an origin only (scheme/host/port), so mounting below `/auth` does not change it. Cross-origin RP-ID metadata remains available from the generated top-level `/.well-known/webauthn` response.
...
Tasker result broadcasts expose short extras `result`, `error`, `status`, and `schema` in addition to the legacy `passkey_*` names. Tasker maps those extras to `%result`, `%error`, `%status`, and `%schema`. Do not attempt to place result payloads in `%evtprm4`: Tasker reserves that event parameter for the Android intent data URI scheme, whose syntax/filter semantics cannot carry arbitrary usernames/passwords safely.
